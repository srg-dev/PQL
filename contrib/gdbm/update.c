     ...
     char *val;
{
  if ((dbf != NULL) && (dbf->fatal_err != NULL))
	...
	write (STDERR_FILENO, val, strlen(val));
      write (STDERR_FILENO, "\n", 1);
	  if (!strcmp(val, "read error"))
	  {
		  //char	*msg = g_strdup("Please delete your ~/.pql directory (and sub-directories) and restart.\n");
		  char	*msg = strdup("Please delete your ~/.pql directory (and sub-directories) and restart.\n");
