...
typedef struct SegyHead {   /*  Offset Description  */
   gint32  lineSeq;           /*   0 Sequence numbers within line */
   gint32  reelSeq;           /*   4 Sequence numbers within reel */
   gint32  event_number;      /*   8 Original field record number or trigger 
                             *   number */
   gint32  channel_number;    /*  12 Trace channel number within the original 
                             *  field record */
   gint32  energySourcePt;    /*  16 X */
   gint32  cdpEns;            /*  20 X */
   gint32  traceInEnsemble;   /*  24 X */
   short traceID;           /*  28 Trace identification code: seismic 
                             *  data = 1 */
   short vertSum;           /*  30 X */
   short horSum;            /*  32 X */
   short dataUse;           /*  34 X */
   gint32  sourceToRecDist;   /*  36 X */
   gint32  recElevation;      /*  40 X */
   gint32  sourceSurfaceElevation; /*  44 X */
   gint32  sourceDepth;       /*  48 X */
   gint32  datumElevRec;      /*  52 X */
   gint32  datumElevSource;   /*  56 X */
   gint32  sourceWaterDepth;  /*  60 X */
   gint32  recWaterDepth;     /*  64 X */
   short elevationScale;    /*  68 Elevation Scaler: scale = 1 */
   short coordScale;        /*  70 Coordinate Scaler: scale = 1 */
   gint32  sourceLongOrX;     /*  72 X */
   gint32  sourceLatOrY;      /*  76 X */
   gint32  recLongOrX;        /*  80 X */
   gint32  recLatOrY;         /*  84 X */
   ...
   short totalStatic;       /* 102 Total Static in MILLISECS added to Trace 
                             * Start Time (lower 2 bytes)*/
   short lagTimeA;          /* 104 X */
   short lagTimeB;          /* 106 X */
   short delay;             /* 108 X */
   short muteStart;         /* 110 X */
   short muteEnd;           /* 112 X */
   short sampleLength;      /* 114 Number of samples in this trace 
                             * (unless == 32767) */
   ...
   short totalStaticHi;     /* 198 Total Static in MILLISECS added to Trace 
                             * Start Time (high 2 bytes)*/
   gint32  samp_rate;         /* 200 Sample interval in MICROSECS as a 32 bit 
                             * integer */
   short data_form;         /* 204 Data Format flag: 0=16 bit, 1=32 bit 
                             * integer */
   ...
   unsigned short inst_no;  /* 224 Instrument Serial Number */
   short not_to_be_used;    /* 226 X */
   gint32  num_samps;         /* 228 Number of Samples as a 32 bit integer
                             * (when sampleLength == 32767) */
   gint32  max;               /* 232 Maximum value in Counts */
   gint32  min;               /* 236 Minimum value in Counts */
