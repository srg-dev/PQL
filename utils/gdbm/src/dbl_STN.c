...
{	// create the data store if not already
	...
	if ((ret=create_dbl_g(dbldi->descr.tableData)) != PQLXSUCCESS)
	{
...
static int read_all(DBLDI *dbldi)
{
	...
	ret = get_all_records_g(dbldi->descr.tableData, dblrecs);
...
static int write_sngl(DBLDI *dbldi)
{
	...
	add_record_g(dbldi->descr.tableData, dblrecs->recs[0]);
...
static int write_all(DBLDI *dbldi)
{
	...
	{
		add_record_g(dbldi->descr.tableData, dblrecs->recs[i]);
	}
...
int dblg_STN(DBLDI *dbldi)
{
	...
	{
		case READ_ALL:
			ret = get_all_records_g(dbldi->descr.tableData, dbldi->data);
		break;
		...
		case DELETE_SNGL:
		{
			...
			{
				case STNCHNFLTRST:
					ret = remove_record_g(STNCHNFLTRST, ((STN_CHN_FILTER *) (dblrecs->recs[0]))->name);
				break;
				case STNCHNYSCALE:
					ret = remove_record_g(STNCHNYSCALE, ((STN_CHN_YSCALE *) (dblrecs->recs[0]))->name);
				break;
				case STNGRPYSCALE:
					ret = remove_record_g(STNGRPYSCALE, ((STN_GRP_YSCALE *) (dblrecs->recs[0]))->name);
				break;
			}
		}
		break;
