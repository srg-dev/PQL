...
{	// create the data store, previous contents are destoryed!
	...
	if ((ret=create_dbl(PDFSRVRST)) != PQLXSUCCESS)
	{
...
static int read_all(DBLRECORDS *dblrecs)
{
	...
	ret = get_all_records(PDFSRVRST, dblrecs);
...
static int write_sngl(DBLRECORDS *dblrecs)
{
	...
	add_record(PDFSRVRST, dblrecs->recs[0]);
...
static int write_all(DBLRECORDS *dblrecs)
{
	...
	{
		add_record(PDFSRVRST, dblrecs->recs[i]);
	}
...
int dbl_pdfsrvr(DBLDI *dbldi)
{
	...
	{
		case READ_ALL:
			ret = get_all_records(PDFSRVRST, dbldi->data);
		break;
		...
		case DELETE_SNGL:
		{
			...
			ret = remove_record(PDFSRVRST, ((PDFSRVR_REC *) (dblrecs->recs[0]))->name);
		}
		break;
	}
