...
static void initDBL()
{
	...
	if ((ret=create_dbl(FILTERST)) != PQLXSUCCESS)
	{
...
static int write_sngl(DBLRECORDS *dblrecs)
{
	...
	add_record(FILTERST, dblrecs->recs[0]);
...
int dbl_filter(DBLDI *dbldi)
{
	...
	{
		case READ_ALL:
			ret = get_all_records(FILTERST, dbldi->data);
		break;
		...
		{
			DBLRECORDS *dblrecs = dbldi->data;
			ret = remove_record(FILTERST, ((FILTER_REC *) (dblrecs->recs[0]))->name);
		}
		break;
	}
